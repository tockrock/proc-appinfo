...
struct AppInfoCLI: ParsableCommand {
    ...
    @Option(name: .customLong("from-pid"), help: "Walk ancestors of this PID instead of the current process.")
    var fromPid: Int32?

    // Single-field selection flags (sw_vers style — mutually exclusive)
    @Flag(name: .long, help: "Print the app name.") var name: Bool = false
    @Flag(name: .customLong("bundleid"), help: "Print the bundle identifier.") var bundleid: Bool = false
    @Flag(name: .long, help: "Print the process ID.") var pid: Bool = false
    @Flag(name: .customLong("bundle-path"), help: "Print the bundle path.") var bundlePath: Bool = false
    @Flag(name: .customLong("executable-path"), help: "Print the executable path.") var executablePath: Bool = false
    @Flag(name: .long, help: "Print the version.") var version: Bool = false
    @Flag(name: .customLong("launch-date"), help: "Print the launch date (ISO 8601).") var launchDate: Bool = false
    @Flag(name: .long, help: "Print whether the app is active.") var active: Bool = false
    @Flag(name: .long, help: "Print whether the app is hidden.") var hidden: Bool = false
    @Flag(name: .customLong("finished-launching"), help: "Print whether the app has finished launching.") var finishedLaunching: Bool = false
    @Flag(name: .customLong("owns-menu-bar"), help: "Print whether the app owns the menu bar.") var ownsMenuBar: Bool = false
    @Flag(name: .customLong("activation-policy"), help: "Print the activation policy.") var activationPolicy: Bool = false
    @Flag(name: .long, help: "Print the executable architecture.") var architecture: Bool = false

    @Flag(name: .long, help: "Output all fields as a JSON object.") var json: Bool = false
    ...
    mutating func run() throws {
        ...
        else if activationPolicy    { print(info.activationPolicy.rawValue) }
        else if architecture        { print(info.architecture.rawValue) }
...
private func humanOutput(_ info: AppInfo) -> String {
    ...
    lines.append(("Architecture:", info.architecture.rawValue))
    ...
    lines.append(("Activation Policy:", info.activationPolicy.rawValue))
